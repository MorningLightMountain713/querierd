...
class Querier:
    ...
    def __init__(self, source_address: str, interval: int):
        ...
        sleep(1)  # Can't set options too soon (???)
        sock.settimeout(5)
    ...
    def run(self) -> None:
        syslog.syslog(f"Querier starting on {self.source_address}")
        wait = 0.0
        timeout = 0.5
        ...
        while True:
            ...
            if self.elected:
                self.socket.sendto(bytes(self.packet), (all_routers, 0))
                if elapsed < self.interval:
                    self.elected = False
                    syslog.syslog(
                        "Lost querier election. Pausing. %s"
                        % self.source_address
                    )
            else:
                if elapsed > 2 * self.interval:
                    syslog.syslog(
                        "Won querier election. Resuming. %s"
                        % self.source_address
                    )
                    self.elected = True
            if not self.listener.thread.is_alive():
                syslog.syslog(
                    "Listener thread died.  Quitting. %s" % self.source_address
                )
                break
...
class QueryListener:
    ...
    def _ip_as_int(self, address: str) -> int:
    ...
    def elapsed(self) -> float:
        ...
        """
        self.lock.acquire()
        result = monotonic() - self._timestamp
        self.lock.release()
        return result
